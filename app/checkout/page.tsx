...
import { useAuth } from "@/core/contexts/AuthContext"; // فرض بر وجود این کانتکست
...
export default function CheckoutPage() {
  ...
  const { user } = useAuth(); // برای پر کردن خودکار نام کاربر (اختیاری)
  ...
  // پر کردن فرم اگر کاربر لاگین باشد (اختیاری)
  useEffect(() => {
  ...
  // هزینه ارسال (ثابت یا محاسبه شده)
  ...
  const onSubmit: SubmitHandler<CheckoutFormInputs> = async (data) => {
    ...
    try {
      ...
      router.push("/checkout/success"); // هدایت به صفحه تشکر
    } catch (error) {
  ...
  if (cartItems.length === 0) return null; // جلوگیری از پرش صفحه قبل از ریدایرکت
    ...
    <div className="bg-gray-50 min-h-screen py-10" dir="rtl">
      <div className="container mx-auto px-4 max-w-6xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          {/* ستون راست: فرم اطلاعات (8 قسمت از 12) */}
          ...
          {/* ستون چپ: خلاصه سفارش (4 قسمت از 12) */}
